...
bool check_swap(Pos a, Pos b) {
  // TODO: Task 1-3
  bool ret = false;
  if( check_inboard(a) == false || check_inboard(b) == false ){
	return false;
  }else if( (fabs(a.x-b.x)>1) || (fabs(a.y-b.y)>1) ){
  	return false;
  }else if( gameboard[a.x][a.y].ability == ABI_KILLSAME && gameboard[b.x][b.y].ability == ABI_BOMB ){
  	return false;
  }else if( gameboard[a.x][a.y].ability == ABI_BOMB && gameboard[b.x][b.y].ability == ABI_KILLSAME ){
  	return false;
  }else if( gameboard[a.x][a.y].ability == ABI_KILLSAME && gameboard[b.x][b.y].ability == ABI_KILLSAME ){
  	return false;
  }else if( gameboard[a.x][a.y].ability == ABI_BOMB && gameboard[b.x][b.y].ability == ABI_BOMB ){
  	return false;
  }else if( gameboard[a.x][a.y].ability == ABI_KILLSAME || gameboard[b.x][b.y].ability == ABI_KILLSAME ){
  	return true;
  }else if( gameboard[a.x][a.y].ability == ABI_BOMB || gameboard[b.x][b.y].ability == ABI_BOMB ){
...
void apply_bomb(Pos pos) {
  ...
  for(int i=-2; i<=2; i++){
  	for(int j=-2; j<=2; j++){
  	  if(check_inboard({(pos.x)+i,(pos.y)+i})==false) continue;
  	  if(gameboard[(pos.x)+i][(pos.y)+j].ability==ABI_NORMAL){
  	  	gameboard[(pos.x)+i][(pos.y)+j].type = GEM_NULL;
        gameboard[(pos.x)+i][(pos.y)+j].ability = ABI_NULL;
	  }else if(gameboard[(pos.x)+i][(pos.y)+j].ability==ABI_BOMB){
	  	apply_bomb({(pos.x)+i,(pos.y)+j});
	  }else if(gameboard[(pos.x)+i][(pos.y)+j].ability==ABI_KILLSAME){
	  	apply_killsame({(pos.x)+i,(pos.y)+j},{-2,gen_rand_type()});
	  }else if(gameboard[(pos.x)+i][(pos.y)+j].ability==ABI_CROSS){
	  	apply_cross({(pos.x)+i,(pos.y)+j});
	  }
	}
  }
}
...
void apply_killsame(Pos pos, Pos tar) {
  // TODO: Task 2-2
  int tar_type = gameboard[tar.x][tar.y].type;
  if((tar.x)<0){
  	tar_type = tar.y;
  }
  for(int i=0; i<BOARD_HEIGHT; i++){
  	for(int j=0; j<BOARD_WIDTH; j++){
  	  if(gameboard[i][j].type==tar_type && gameboard[i][j].ability==ABI_NORMAL){
  	  	gameboard[i][j].type = GEM_NULL;
        gameboard[i][j].ability = ABI_NULL;
	  }else if(gameboard[i][j].type==tar_type && gameboard[i][j].ability==ABI_CROSS){
	  	apply_cross({i,j});
	  } 
	}
  }
...
void apply_cross(Pos pos) {
  // TODO: Task 2-3
  int cross_type = gameboard[pos.x][pos.y].type;
  gameboard[pos.x][pos.y].type = GEM_NULL;
  gameboard[pos.x][pos.y].ability = ABI_NULL;
  for(int i=0; i<BOARD_HEIGHT; i++){
  	if(gameboard[i][pos.y].ability==ABI_NORMAL){
  	  gameboard[i][pos.y].type = GEM_NULL;
      gameboard[i][pos.y].ability = ABI_NULL;
    }else if(gameboard[i][pos.y].ability==ABI_BOMB){
      apply_bomb({i,pos.y});
	}else if(gameboard[i][pos.y].ability==ABI_KILLSAME){
	  apply_killsame({i,pos.y},{-1,cross_type});
	}else if(gameboard[i][pos.y].ability==ABI_CROSS){
	  apply_cross({i,pos.y});
	}
  }
  for(int i=0; i<BOARD_WIDTH; i++){
  	if(gameboard[pos.x][i].ability==ABI_NORMAL){
  	  gameboard[pos.x][i].type = GEM_NULL;
      gameboard[pos.x][i].ability = ABI_NULL;
    }else if(gameboard[pos.x][i].ability==ABI_BOMB){
      apply_bomb({pos.x,i});
	}else if(gameboard[pos.x][i].ability==ABI_KILLSAME){
	  apply_killsame({pos.x,i},{-1,cross_type});
	}else if(gameboard[pos.x][i].ability==ABI_CROSS){
	  apply_cross({pos.x,i});
	}
  }
}
...
void apply_special(Pos pos, Pos tar) {
  ...
  if(gameboard[pos.x][pos.y].ability==ABI_BOMB){
  	apply_bomb(pos);
  }else if(gameboard[pos.x][pos.y].ability==ABI_KILLSAME){
  	apply_killsame(pos,tar);
  }else if(gameboard[pos.x][pos.y].ability==ABI_CROSS){
  	apply_cross(pos);
  }
}
